...
const SLUG = "immunity-vitamin-c" as const;
...
const relatedArticlesQO = queryOptions({ queryKey: ["insights-by-slugs", "vitamin-c-immunity-energy"], queryFn: () => listInsightsBySlugs({ data: { slugs: ["vitamin-c-immunity-energy"] } }) });
...
export const Route = createFileRoute("/$lang/immunity-vitamin-c")({
  head: ({ params, loaderData }) => {
    ...
    const collection = {
      ...
      about: [
        { "@type": "Thing", name: isAr ? "فيتامين C" : "Vitamin C" },
      ...
      <section className="cinema-hero relative overflow-hidden">
        <div className="mx-auto max-w-7xl px-4 py-16 md:px-8 md:py-24">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <LLink to="/$lang/brands/$slug" params={{ slug: "monivo" }} className="inline-flex items-center justify-center rounded-full border border-border bg-card px-6 py-3 text-sm font-semibold text-foreground transition-colors hover:border-trust-700 hover:text-trust-700">
              {T("hero.link1Label")}
            </LLink>
          </div>
        </div>
                ...
                return (
                  <LLink key={p.id} to="/$lang/brands/$slug/$productSlug" params={{ slug: "monivo", productSlug: p.slug }} className="prem-card group flex flex-col">
                    <figure className="podium relative grid aspect-[4/3] place-items-center p-6">
                      {p.cover_url ? (
                        <img src={p.cover_url} alt={productAlt("monivo", bname, pname, isAr ? "ar" : "en")} loading="lazy" className="max-h-full w-auto object-contain" />
                      ) : null}
                    </figure>
